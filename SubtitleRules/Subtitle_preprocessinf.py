    ...
    for item in data:
        ...
        if isinstance(parsed_events, list):
            for event in parsed_events:
                ...
                # Handle None and list values
                event_type_str = str(event_type) if event_type else ""
                player_str = str(player[0] if isinstance(player, list) and player else player) if player else ""
                team_str = str(team[0] if isinstance(team, list) and team else team) if team else ""
...
def main():
    ...
    stl_file_path = "/Users/prda5207/PycharmProjects/Labweek_Fall_2025_MatchAgent/SubtitleRules/Data/Subtitles_Aigenerated.stl"
